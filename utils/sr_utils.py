...
def load_LR_HR_imgs_sr(fname, imsize, factor, enforse_div32=None):
    ...
    ######Changes by Marcos#####
    ft = np.fft.fftshift(np.fft.fft2(img_orig_np, norm = "forward")) #img
    C,H,W = (ft.shape)
    ...
    ift = np.abs(np.fft.ifft2(ftcrop,norm='forward')) #inverse transfrom
    img_LR_np = ift
...
def get_baselines(img_LR_pil, img_HR_pil):
    ...
    #Changes
    img_LRs = img_LR_pil.convert('RGB')
    img_HRs = img_HR_pil.convert('RGB')
    
    img_bicubic_pil2 = img_LRs.resize(img_HRs.size, Image.BICUBIC)
    img_bicubic_np2 = pil_to_np(img_bicubic_pil2)

    img_bic_sharp_pil = img_bicubic_pil2.filter(PIL.ImageFilter.UnsharpMask())
